<?php
require_once $_SERVER['DOCUMENT_ROOT'] . "/class/dao/contabil/pagamento/DaoConPagamentoHistorico.class.php";

class ConPagamentoHistorico {
    ...
    public function salvaHistoricoPagamento(PDO $pdo) {
        ...
        if(!empty($this->id_pagamento) && !empty($this->id_pessoa) && !empty($this->id_lotacao) && !empty($this->id_doc_tipo_lotacao) && !empty($this->id_pagamento_situacao) && 
           !empty($this->id_pagamento_status)){
            
            $daoConPagamentoHistorico = new DaoConPagamentoHistorico();
            $daoConPagamentoHistorico->setIdPagamento($this->id_pagamento);
            $daoConPagamentoHistorico->setIdPessoa($this->id_pessoa);
            $daoConPagamentoHistorico->setIdLotacao($this->id_lotacao);
            $daoConPagamentoHistorico->setIdDocTipoLotacao($this->id_doc_tipo_lotacao);
            $daoConPagamentoHistorico->setIdPagamentoSituacao($this->id_pagamento_situacao);
            $daoConPagamentoHistorico->setIdPagamentoStatus($this->id_pagamento_status);
            $daoConPagamentoHistorico->setDhPagamentoHistorico($this->dh_pagamento_historico);
            $daoConPagamentoHistorico->setDsPagamentoHistorico($this->ds_pagamento_historico);
            $daoConPagamentoHistorico->salvaHistorico($pdo);
            if($daoConPagamentoHistorico->Sucesso()){
                $this->sucesso = true;
            }else{
                $this->sucesso = false;
                $this->msgRetorno = $daoConPagamentoHistorico->getMsgRetorno();
            }
        }else{
            $this->sucesso = false;
