...
class DaoFinEntregaConfirmacao extends FinEntregaConfirmacaoTb {
    ...
    public function retornaInforParaEntrega(PDO $pdo) {
        try {
            if ($pdo != null) {
                $sql = "select itens.nr_item, mat.nm_material, mat.cd_desc_material,
                        case 
                         when itens.ds_itens is not null then itens.ds_itens
                         else mat.nm_desc_material
                        end nm_desc_material, desp.cd_despesa, desp.ds_despesa, mat.tp_material,
                        itens.nr_lote, ordemItens.qt_itens_ordem, ordemItens.vl_itens_ordem,
                        pessoa.nm_pessoa, p.nr_pedido, p.ds_pedido, cont.nr_contrato, cont.tp_contrato,
                        ordem.nr_ordem, ordem.aa_ordem, emp.nr_empenho 
