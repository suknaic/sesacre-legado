...
class DaoFinEntregaConfirmacao extends FinEntregaConfirmacaoTb {
    ...
    public function retornaInforParaEntrega(PDO $pdo) {
        try {
            if ($pdo != null) {
                $sql = "select entregaC.id_entrega_confirmacao, ordemItens.id_ordem_itens, itens.nr_item, 
                        mat.nm_material, mat.cd_desc_material,
                        case 
                                when itens.ds_itens is not null then itens.ds_itens
                            else mat.nm_desc_material
                        end nm_desc_material, desp.cd_despesa, desp.ds_despesa, mat.tp_material, itens.fl_valor_variavel,
                        pro.ds_protocolo, itens.nr_lote, pessoa.nm_pessoa, p.nr_pedido, p.ds_pedido, cont.nr_contrato, 
                        cont.tp_contrato, ordem.nr_ordem, ordem.aa_ordem, emp.nr_empenho, 
                        to_char(ordemItens.qt_itens_ordem, '9G999G990D9999')as qt_itens_ordem,
                        ...
                        case 
                            ...
                            then (select coalesce(to_char(sum(qt_itens_entrega), '9G999G990D9999'),'0,0000') 
                            ...
                            then (select coalesce(to_char(sum(qt_itens_entrega * vl_itens_entrega), '9G999G990D9999'),'0,0000') 
                                          from fin_entrega_itens 
                                          where id_entrega_confirmacao = entregaC.id_entrega_confirmacao
                                          and id_ordem_itens = ordemItens.id_ordem_itens)
