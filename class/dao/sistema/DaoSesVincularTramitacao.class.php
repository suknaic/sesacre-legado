...
class DaoSesVincularTramitacao extends SesVincularTramitacao {
    ...
    function retornaLotacaoTipoEmpenhoPorUsuario(PDO $pdo = null) {
        try {
            $sql = "select distinct
                     ...
                     and svt.id_tramitacao = 1 --Empenhar";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(":id_pessoa", $this->getIdPessoa(), PDO::PARAM_INT);
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
            ...
            $sql = "select distinct
                     ...
                     from
                        ...
                        inner join
                           ...
                           on tipoLot.id_doc_tipo_lotacao = svt.id_doc_tipo_lotacao 
                     where svt.id_pessoa = :id_pessoa
                     and svt.id_tramitacao = 6 --Reativação da Ordem";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(":id_pessoa", $this->getIdPessoa(), PDO::PARAM_INT);
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
    ...
    function retornaLotacaoTipoAutorizacaoReativacaoOrdemPorUsuario(PDO $pdo = null) {
        try {
            $sql = "select distinct
                        svt.id_lotacao,
                        svt.id_doc_tipo_lotacao,
                     ...
                     from
                        ...
                        inner join
                           ...
                           on tipoLot.id_doc_tipo_lotacao = svt.id_doc_tipo_lotacao 
                     where svt.id_pessoa = :id_pessoa
                     and svt.id_tramitacao = 7 --Autorização da Reativação da Ordem";
            $stmt = $pdo->prepare($sql);
            $stmt->bindValue(":id_pessoa", $this->getIdPessoa(), PDO::PARAM_INT);
            $stmt->execute();
            if ($stmt->rowCount() > 0) {
