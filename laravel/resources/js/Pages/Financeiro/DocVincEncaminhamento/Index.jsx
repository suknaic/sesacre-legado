...
export default function Index({ vincos, filters, lotacoes }) {
    ...
    function handleFilter(e) {
        ...
        setParams(next);
        router.get(route('financeiro.doc-vinc-encaminhamentos.index'), next, { preserveState: true, replace: true });
    }

    function handleDelete(id) {
        if (confirm('Remover este vínculo de encaminhamento?')) {
            router.delete(route('financeiro.doc-vinc-encaminhamentos.destroy', id));
        }
    }
    ...
    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Vínculos de Encaminhamento</h2>
                <Link href={route('financeiro.doc-vinc-encaminhamentos.create')} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">Novo Vínculo</Link>
            </div>
        }>
            <Head title="Vínculos de Encaminhamento" />
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {vincos.data.length === 0 ? (
                                ...
                                <tr key={v.id_doc_vinc_encaminhamento}>
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                                        <button onClick={() => handleDelete(v.id_doc_vinc_encaminhamento)} className="text-red-600 hover:text-red-900">Remover</button>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
