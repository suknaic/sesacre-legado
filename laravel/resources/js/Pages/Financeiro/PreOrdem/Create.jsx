...
export default function Create({ pedidos, fornecedores, contratos, tiposGasto }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route('financeiro.pre-ordens.store'));
    }

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Nova Pré-Ordem</h2>}>
            <Head title="Nova Pré-Ordem" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                ...
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        ...
                        <div className="mb-4">
                            <label className="block text-sm font-medium text-gray-700">Item de Contrato</label>
                            <select value={data.id_cont_itens} onChange={e => setData('id_cont_itens', e.target.value)}
                                className="mt-1 block w-full rounded-md border-gray-300 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                <option value="">Sem item específico</option>
                                {contratos.map(c => <option key={c.id_contrato} value="">{c.nr_contrato}</option>)}
                            </select>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('financeiro.pre-ordens.index')} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
