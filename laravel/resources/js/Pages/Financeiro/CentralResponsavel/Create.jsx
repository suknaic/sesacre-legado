...
export default function Create({ lotacoes, tiposAdministracao }) {
    const { data, setData, post, processing, errors } = useForm({
        id_lotacao: '',
        id_pessoa: '',
        id_tipo_administracao: '',
    });

    function handleSubmit(e) {
        e.preventDefault();
        post(route('financeiro.centrais-responsavel.store'));
    }

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Novo Responsável por Central</h2>}>
                ...
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('financeiro.centrais-responsavel.index')} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
