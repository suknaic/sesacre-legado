...
export default function Create({ lotacoes }) {
    ...
    function handleSubmit(e) {
        e.preventDefault();
        post(route('financeiro.doc-vinc-recebimentos.store'));
    }

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Novo Vínculo de Recebimento</h2>}>
            <Head title="Novo Vínculo de Recebimento" />
            <div className="py-12"><div className="mx-auto max-w-2xl sm:px-6 lg:px-8">
                <div className="mb-4"><Link href={route('financeiro.doc-vinc-recebimentos.index')} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('financeiro.doc-vinc-recebimentos.index')} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
