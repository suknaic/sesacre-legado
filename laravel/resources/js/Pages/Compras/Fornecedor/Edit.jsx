...
export default function Edit({ fornecedor, medicamentos, servicos, materiaisConsumo, materiaisPermanente }) {
    const { data, setData, put, processing, errors } = useForm({
        nm_pessoa: fornecedor.pessoa?.nm_pessoa || '',
        nr_cpf_cnpj: fornecedor.documento_raw || '',
        nr_telefone_celular: fornecedor.pessoa?.nr_telefone_celular || '',
        nm_email: fornecedor.pessoa?.nm_email || '',
        ds_logradouro: fornecedor.pessoa?.ds_logradouro || '',
        ds_bairro: fornecedor.pessoa?.ds_bairro || '',
        nr_cep: fornecedor.pessoa?.nr_cep || '',
        id_cidade: fornecedor.pessoa?.id_cidade || '',
        sit_fornecedor: fornecedor.sit_fornecedor,
        medicamentos: fornecedor.medicamentos?.map(m => m.id_medicamento) || [],
        servicos: fornecedor.servicos?.map(s => s.id_servico) || [],
        materiais_consumo: fornecedor.materiais_consumo?.map(m => m.id_material_consumo) || [],
        materiais_permanente: fornecedor.materiais_permanente?.map(m => m.id_material_permanente) || [],
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('compras.fornecedores.update', fornecedor.id_fornecedor));
    }
    ...
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Fornecedor</h2>}>
            <Head title="Editar Fornecedor" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="mb-4"><Link href={route('compras.fornecedores.show', fornecedor.id_fornecedor)} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        ...
                        <div className="flex items-center justify-end gap-4 mt-6">
                            <Link href={route('compras.fornecedores.show', fornecedor.id_fornecedor)} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
