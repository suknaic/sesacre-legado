...
export default function Index({ materiais, filters }) {
    ...
    function handleFilter(e) {
        ...
        setParams(next);
        router.get(route('compras.materiais-permanente.index'), next, { preserveState: true, replace: true });
    }

    function clearFilters() {
        setParams({});
        router.get(route('compras.materiais-permanente.index'), {}, { preserveState: true, replace: true });
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Materiais Permanentes</h2>
                <Link href={route('compras.materiais-permanente.create')} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">Novo Material</Link>
            </div>
        }>
            <Head title="Materiais Permanentes" />
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {materiais.data.length === 0 ? (
                                ...
                                <tr key={m.id_material_permanente}>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{m.id_material_permanente}</td>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-900">{m.nm_material_permanente}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
