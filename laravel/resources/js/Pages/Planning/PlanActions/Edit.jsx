...
export default function Edit({ planAction, objectives }) {
    const { data, setData, put, processing, errors } = useForm({
        plan_objective_id: planAction.plan_objective_id || '',
        name: planAction.name,
        indicator: planAction.indicator || '',
        goal: planAction.goal || '',
        registration_type: planAction.registration_type || '',
        is_active: planAction.is_active,
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('plan-actions.update', planAction.id));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Ação</h2>
