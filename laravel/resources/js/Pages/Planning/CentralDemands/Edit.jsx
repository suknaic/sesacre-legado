...
export default function Edit({ demand, people, organizations }) {
    const { data, setData, put, processing, errors } = useForm({
        personal_info_id: demand.personal_info_id?.toString() || '',
        organization_id: demand.organization_id?.toString() || '',
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('central-demands.update', demand.id));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Central de Demanda</h2>
