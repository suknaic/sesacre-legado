...
export default function Edit({ responsible, people, organizations }) {
    const { data, setData, put, processing, errors } = useForm({
        personal_info_id: responsible.personal_info_id?.toString() || '',
        organization_id: responsible.organization_id?.toString() || '',
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('pas-responsibles.update', responsible.id));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Responsável PAS</h2>
            ...
            <Head title="Editar Responsável PAS" />
            <div className="py-8">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Link href={route('pas-responsibles.index')} className="text-sm text-gray-600 hover:text-gray-900">Cancelar</Link>
                            </div>
                        </form>
                    </div>
                </div>
