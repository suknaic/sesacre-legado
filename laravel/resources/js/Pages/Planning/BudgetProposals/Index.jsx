...
export default function Index({ budgetProposals }) {
    const { flash } = usePage().props;
    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Propostas Orçamentárias</h2>
                <Link href={route('budget-proposals.create')} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">Nova Proposta</Link>
            </div>
        }>
            <Head title="Propostas Orçamentárias" />
            <div className="py-8">
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <table className="min-w-full divide-y divide-gray-200">
                            ...
                            <tbody className="divide-y divide-gray-200 bg-white">
                                {budgetProposals.data.length === 0 ? (
                                    <tr><td colSpan="4" className="px-6 py-4 text-center text-sm text-gray-500">Nenhuma proposta encontrada.</td></tr>
                                    ...
                                    <tr key={p.id}>
                                        ...
                                        <td className="whitespace-nowrap px-6 py-4 text-sm">
                                            ...
                                            <Link href={route('budget-proposals.edit', p.id)} className="ml-3 text-indigo-600 hover:text-indigo-900">Editar</Link>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
