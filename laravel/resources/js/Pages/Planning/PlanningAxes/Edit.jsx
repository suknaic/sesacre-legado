...
export default function Edit({ axis, pesPlans }) {
    const { data, setData, put, processing, errors } = useForm({
        pes_plan_id: axis.pes_plan_id?.toString() || '',
        name: axis.name || '',
        order: axis.order?.toString() || '0',
        is_active: axis.is_active,
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('planning-axes.update', axis.id));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Eixo</h2>
            ...
            <Head title="Editar Eixo" />
            <div className="py-8">
                <div className="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-gray-700">PES</label>
                                <select value={data.pes_plan_id} onChange={e => setData('pes_plan_id', e.target.value)} className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                    <option value="">Selecione...</option>
                                    {pesPlans.map((p) => (
                                        <option key={p.id} value={p.id}>{p.name}</option>
                                    ))}
                                </select>
                                {errors.pes_plan_id && <p className="mt-1 text-sm text-red-600">{errors.pes_plan_id}</p>}
                            </div>
                            <div>
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Link href={route('planning-axes.index')} className="text-sm text-gray-600 hover:text-gray-900">Cancelar</Link>
                            </div>
                        </form>
                    </div>
                </div>
