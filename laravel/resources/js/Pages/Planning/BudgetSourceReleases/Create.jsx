...
export default function Create({ fontes }) {
    const { data, setData, post, processing, errors } = useForm({
        fonte_id: '', year: '', total_amount: '',
    });

    function handleSubmit(e) {
        e.preventDefault();
        post(route('budget-source-releases.store'));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Nova Liberação de Fonte</h2>
            ...
            <Head title="Nova Liberação de Fonte" />
            <div className="py-8">
