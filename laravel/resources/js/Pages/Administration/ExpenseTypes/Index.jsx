    ...
    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Tipos de Despesa</h2>
                <Link href={route('expense-types.create')} className="rounded-md bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-indigo-500">Novo Tipo de Despesa</Link>
            </div>
        }>
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {expenseTypes.data.length === 0 ? (
                                <tr><td colSpan="4" className="px-6 py-4 text-center text-sm text-gray-500">Nenhum tipo de despesa encontrado.</td></tr>
                            ) : expenseTypes.data.map((p) => (
                                <tr key={p.id}>
                                    ...
                                    <td className="whitespace-nowrap px-6 py-4 text-sm">
                                        <Link href={route('expense-types.show', p.id)} className="text-indigo-600 hover:text-indigo-900">Ver</Link>
                                        <Link href={route('expense-types.edit', p.id)} className="ml-3 text-indigo-600 hover:text-indigo-900">Editar</Link>
                                    </td>
                                </tr>
                            ))}
                        </tbody>
