...
export default function Edit({ jobFunction }) {
    const { data, setData, put, processing, errors } = useForm({ name: jobFunction.name, is_active: jobFunction.is_active });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('job-functions.update', jobFunction.id));
    }

    return (
        <AuthenticatedLayout header={
            <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Função</h2>
                <Link href={route('job-functions.index')} className="rounded-md bg-gray-600 px-4 py-2 text-sm font-semibold text-white shadow hover:bg-gray-500">Voltar</Link>
            </div>
        }>
                    ...
                    <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                        <form onSubmit={handleSubmit} className="p-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4">
                                ...
                                <Link href={route('job-functions.index')} className="text-sm text-gray-600 hover:text-gray-900">Cancelar</Link>
                            </div>
                        </form>
                    </div>
                </div>
