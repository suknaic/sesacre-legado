...
export default function Index({ personalInfos }) {
                ...
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50"><tr>
                            ...
                            <th className="px-6 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500">Data Nasc.</th>
                        ...
                        <tbody className="divide-y divide-gray-200 bg-white">
                            {personalInfos.data.length === 0 ? (
                                <tr><td colSpan="6" className="px-6 py-4 text-center text-sm text-gray-500">Nenhum funcionário encontrado.</td></tr>
                            ) : personalInfos.data.map((p) => (
                                <tr key={p.id}>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900">{p.user?.name || '-'}</td>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{p.rg || '-'}</td>
                                    <td className="whitespace-nowrap px-6 py-4 text-sm text-gray-500">{p.user?.cpf || '-'}</td>
