...
export default function Create({ employmentContract, organizations = [], jobFunctions = [] }) {
    const { flash } = usePage().props;
    const { data, setData, post, processing, errors } = useForm({
        organization_id: '', job_function_id: '', workload: '', start_date: '', end_date: '',
    });

    function handleSubmit(e) { e.preventDefault(); post(route('employment-contracts.locations.store', employmentContract.id)); }

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Nova Lotação - {employmentContract.personal_info?.user?.name}</h2>}>
            <Head title="Nova Lotação" />
