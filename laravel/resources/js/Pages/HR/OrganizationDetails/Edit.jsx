...
export default function Edit({ organizationDetail, parents, categories, cities, managers }) {
    const { flash } = usePage().props;
    const d = organizationDetail;
    const { data, setData, put, processing, errors } = useForm({
        parent_id: d.parent_id || '', category_id: d.category_id || '',
        name: d.name || '', cnpj: d.cnpj || '',
        city_id: d.city_id || '', address: d.address || '',
        neighborhood: d.neighborhood || '', zip_code: d.zip_code || '',
        email: d.email || '', phone: d.phone || '',
        latitude: d.latitude || '', longitude: d.longitude || '',
        manager_id: d.manager_id || '',
        is_principal: d.is_principal || false, is_active: d.is_active ?? true,
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('organization-details.update', d.id));
    }

    return (
        <AuthenticatedLayout header={
            <h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Detalhe de Organização</h2>
        }>
            <Head title="Editar Detalhe de Organização" />
