...
export default function Edit({ portaria, redes }) {
    const { data, setData, put, processing, errors } = useForm({
        id_rede_tematica: String(portaria.id_rede_tematica || ''),
        nm_portaria: portaria.nm_portaria || '',
        dt_portaria: portaria.dt_portaria || '',
        vl_total: String(portaria.vl_total || ''),
        st_portaria: String(portaria.st_portaria || '1'),
    });
    function handleSubmit(e) { e.preventDefault(); put(route('orcamento.portarias.update', portaria.id_portaria)); }
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar {portaria.nm_portaria}</h2>}>
            <Head title="Editar Portaria" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="mb-6"><Link href={route('orcamento.portarias.show', portaria.id_portaria)} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                        <div className="mb-4">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('orcamento.portarias.show', portaria.id_portaria)} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
