...
export default function Create({ blocos }) {
    const { data, setData, post, processing, errors } = useForm({
        nm_rede_tematica: '', id_bloc_orcamentario: '',
    });
    function handleSubmit(e) { e.preventDefault(); post(route('orcamento.redes-tematicas.store')); }
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Nova Rede Temática</h2>}>
            <Head title="Nova Rede Temática" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="mb-6"><Link href={route('orcamento.redes-tematicas.index')} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                        <div className="mb-4">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('orcamento.redes-tematicas.index')} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
