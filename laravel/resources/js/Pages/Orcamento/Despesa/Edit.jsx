...
export default function Edit({ despesa }) {
    const { data, setData, put, processing, errors } = useForm({
        cd_despesa: despesa.cd_despesa || '',
        nm_despesa: despesa.nm_despesa || '',
        st_ativo: String(despesa.st_ativo ?? '1'),
    });
    function handleSubmit(e) { e.preventDefault(); put(route('orcamento.despesas.update', despesa.id_despesa)); }
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar {despesa.nm_despesa}</h2>}>
            <Head title="Editar Despesa" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="mb-6"><Link href={route('orcamento.despesas.show', despesa.id_despesa)} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                        <div className="mb-4">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('orcamento.despesas.show', despesa.id_despesa)} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
