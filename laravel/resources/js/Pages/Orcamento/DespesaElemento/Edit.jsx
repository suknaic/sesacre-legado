...
export default function Edit({ despesaElemento, despesas }) {
    const { data, setData, put, processing, errors } = useForm({
        cd_despesa_elemento: despesaElemento.cd_despesa_elemento || '',
        nm_despesa_elemento: despesaElemento.nm_despesa_elemento || '',
        id_despesa: String(despesaElemento.id_despesa || ''),
        st_ativo: String(despesaElemento.st_ativo ?? '1'),
    });
    function handleSubmit(e) { e.preventDefault(); put(route('orcamento.despesa-elementos.update', despesaElemento.id_despesa_elemento)); }
    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar {despesaElemento.nm_despesa_elemento}</h2>}>
            <Head title="Editar Elemento de Despesa" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="mb-6"><Link href={route('orcamento.despesa-elementos.show', despesaElemento.id_despesa_elemento)} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                        <div className="mb-4">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('orcamento.despesa-elementos.show', despesaElemento.id_despesa_elemento)} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
