...
export default function Edit({ perDiemRequest, travelTypes, transportTypes, decreeTypes, travelClasses }) {
    const { data, setData, put, processing, errors } = useForm({
        service_description: perDiemRequest.service_description || '',
        travel_type_id: perDiemRequest.travel_type_id || '',
        transport_type_id: perDiemRequest.transport_type_id || '',
        decree_type_id: perDiemRequest.decree_type_id || '',
        travel_class_id: perDiemRequest.travel_class_id || '',
        locations: perDiemRequest.locations || '',
        notes: perDiemRequest.notes || '',
        is_active: perDiemRequest.is_active ?? true,
    });

    function handleSubmit(e) {
        e.preventDefault();
        put(route('per-diem-requests.update', perDiemRequest.id));
    }

    return (
        <AuthenticatedLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Editar Solicitação de Diária #{perDiemRequest.id}</h2>}>
            <Head title="Editar Solicitação de Diária" />
            <div className="py-12"><div className="mx-auto max-w-3xl sm:px-6 lg:px-8">
                <div className="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <form onSubmit={handleSubmit} className="p-6">
                        <div className="mb-6"><Link href={route('per-diem-requests.show', perDiemRequest.id)} className="text-sm text-indigo-600 hover:text-indigo-900">&larr; Voltar</Link></div>
                        ...
                        <div className="flex items-center justify-end gap-4">
                            <Link href={route('per-diem-requests.show', perDiemRequest.id)} className="rounded-md bg-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-300">Cancelar</Link>
